...
if ($_SERVER["REQUEST_METHOD"]=="POST") {
    ...
    if (isset($_POST['sellingp'])) $sellingp = $_POST['sellingp'];
    if (isset($_POST['disc'])) $disc = $_POST['disc'];
    $error=array();
    if (empty($_POST["oquantity"])) {
        $error[]='Please update the Outgoing Quantity';
    }
    if (empty($_POST["sellingp"])) {
        $error[]='Please update the Selling Price';
    }
    if (empty($_POST["disc"])) {
        $error[]='Please update the Discount';
    }
}
$uQuery="select S.Stockid,S.ProductName,S.ExpiryDate,O.DateOut,S.BatchNumber,S.buyingprice,S.sellingprice,S.discount,O.OutgoingQuantity FROM stock S,outgoingstock O WHERE S.Stockid=O.Stockid AND S.Stockid='$osid' AND S.Storeid='$sid'";
                ...
                <div class="panel-body">
                    <form role="form" method="post" action="">
                        ...
                        <div class="form-group">
                            ...
                            <input name="sellingp" type="text" class="form-control" id="sellingp" placeholder="<?php echo $sp?>" value="<?php if (isset($_POST['sellingp'])) $sellingp = $_POST['sellingp'];?>">
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">Discount</label>
                            <input name="disc" type="text" class="form-control" id="disc" placeholder="<?php echo $dis?>" value="<?php if (isset($_POST['disc'])) $disc = $_POST['disc'];?>">
                        </div>
                    ...
                    if (isset($error)) {
                        if (!empty($error)) {
                            ...
                            $utotal=($sellingp-$disc)*$oquantity;
                            $ingoing=$totalStock;
                            ...
                            if($sdiff<=$minquan){
                                ...
                                //update query here
                                $insertQuery = "UPDATE `stockmanagement`.`outgoingstock` SET `OutgoingQuantity` = '" . $oquantity . "',`outgoingtotal` = '" . $utotal . "' WHERE `outgoingstock`.`Stockid` =$osid;";
                                $outgoingQuery = "UPDATE `stockmanagement`.`ingoingstock` SET `IngoingQuantity` = '" . $ingoingStock . "' WHERE `ingoingstock`.`Stockid` =$osid;";
                                $updateStocki = "UPDATE `stock` SET `ingoing` = '" . $sdiff . "', `sellingprice` = '" . $sellingp . "', `discount` = '" . $disc . "' WHERE `stock`.`Stockid` = '" . $osid . "';";
